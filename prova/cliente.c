...
char * parse_m2s(float **m, int i, int j){
...
//	signal(SIGINT,interromper);
	...
	for(i=0; i<h_img; i++)
		for(j=0; j<w_img; j++){
			temp1=binarizar(img,i,j);
			erro(img,i,j,temp1);
		}

	escreve_img("imagem_final.y");
